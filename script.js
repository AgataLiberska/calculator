...
function formatResult(result) {
    const output_max_length = 10;
    const output_precision = 5;

    if ( digitCounter(result) > output_max_length) {
        if (isFloat(result)) {
            const result_int = parseInt(result);
            const result_int_length = digitCounter(result_int);

            if (result_int_length > output_max_length) {
                return result.toPrecision(output_precision);
            }            
            else {
                const num_of_digits_after_point = output_max_length - result_int_length;
